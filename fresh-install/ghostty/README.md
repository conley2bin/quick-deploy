...
- `~/.config/ghostty/config.ghostty`
  ...
  - `F11` 全屏切换
  - `Ctrl+Alt+←/→` 切标签、`Ctrl+Alt+=` 新建标签（所有权归 Ghostty 标签页）
...
### Ctrl+Alt+←/→ 与 Ctrl+Alt+= 的所有权：Ghostty 标签页

这组键归 Ghostty 标签页，模块显式写：

```ini
keybind = ctrl+alt+arrow_left=previous_tab
keybind = ctrl+alt+arrow_right=next_tab
keybind = ctrl+alt+equal=new_tab
keybind = ctrl+alt+plus=new_tab
```

`=` 与 `+` 同键：不按/按住 Shift 分别产生 equal/plus，两个都绑。新标签继承
当前目录（`tab-inherit-working-directory` 默认 true，依赖 shell integration）。

两个上游行为差异：

- `previous_tab`/`next_tab` **不环绕**（上游刻意决定，issue #999），到头即停；
- `new_tab` 没有「先问名字」的机制，改名只能建后 `prompt_tab_title`。

历史：这组键一度被 unbind 让给 tmux 切 window（tmux.conf.local 在 root 表接
`C-M-Left/Right`），现归还标签页，tmux 侧绑定已同步移除；tmux 内切 window 改用
`前缀 Ctrl-H/L`，新建用 `前缀 c`。
