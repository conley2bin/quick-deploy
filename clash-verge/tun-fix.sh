...
get_registered_script_config() {
    ...
    mapfile -t script_files < <(awk '
        $0 == "- uid: Script" {found=1; is_script=0; next}
        found && /^  type: script$/ {is_script=1; next}
        found && /^  file: / && is_script {print $2; found=0; next}
        found && /^- uid: / {found=0}
    ' "$PROFILES_YAML")
    if [ "${#script_files[@]}" -ne 1 ] || [ -z "${script_files[0]:-}" ]; then
        echo "需要唯一且已登记的全局 Script 目标；拒绝写入不会被 Verge 加载的孤儿或歧义文件。" >&2
        return 1
    fi
    script_file="${script_files[0]}"
    if [[ "$script_file" = */* || "$script_file" = .* || "$script_file" = *".."* ]]; then
        echo "profiles.yaml 中的全局 Script 文件名不安全: $script_file" >&2
        return 1
    fi
    echo "$CLASH_DIR/profiles/$script_file"
}
...
# 更新或创建直连规则
# Generate candidate first, then preserve the existing ownership/backup contract.
# This path only touches the registered global Script extension; it never edits
# Merge, DNS, TUN, SSH, subscription extensions, runtime YAML, or reloads Verge.
apply_route_rules() {
    local script_file timestamp candidate backed_up=0

    require_profiles
    script_file=$(get_registered_script_config)
    candidate=$(mktemp "${TMPDIR:-/tmp}/tun-fix-rules.XXXXXX")
    trap 'rm -f "$candidate"' RETURN
    render_route_script > "$candidate"
    ...
    mv "$candidate" "$script_file"
    trap - RETURN
    echo "路由规则已写入已登记的全局脚本: $script_file"
    if [ "$backed_up" -eq 1 ]; then
        echo "脚本备份: $script_file.backup.$timestamp"
    fi
}

# Kept as the full optimizer's existing integration seam.
update_direct_rules() {
    apply_route_rules
}
...
optimize_all() {
    ...
    # Do not let an invalid local route source be discovered after Merge/DNS/TUN
    # writes have already begun. Rendering itself is still repeated by the final
    # Script replacement so the candidate always precedes its backup.
    check_route_sources
