...
apply_route_rules() {
    local script_file target_dir candidate backup_file backed_up=0

    require_profiles
    # This also validates sources and reports bound per-profile Rules duplicates
    # before a candidate, backup, or registered Script is touched.
    check_route_migration
    ...
    candidate=$(mktemp "$target_dir/.${script_file##*/}.candidate.XXXXXX")
    ...
    if [ -f "$script_file" ]; then
        backup_file=$(mktemp "$script_file.backup.XXXXXX")
    ...
    # Candidate and target share a directory, so replacement is atomic and never
    # crosses filesystems. Candidate was rendered before any backup/replacement.
    mv -f -- "$candidate" "$script_file"
