...
prepare_route_rules() {
    local script_file target_dir candidate snapshot
    ...
    snapshot=$(mktemp "$target_dir/.${script_file##*/}.rules.XXXXXX") || {
        rm -f -- "$candidate"
        return 1
    }
    if ! python3 "$RULES_READER" \
        ...
        --prepared-rules "$snapshot" \
        --prepare; then
        rm -f -- "$candidate" "$snapshot"
        return 1
    fi
    ...
    PREPARED_ROUTE_RULES="$snapshot"
}

write_prepared_route_rules() {
    local backup_file=""

    if [ -z "$PREPARED_ROUTE_SCRIPT" ] || [ -z "$PREPARED_ROUTE_CANDIDATE" ] || [ -z "$PREPARED_ROUTE_RULES" ]; then
