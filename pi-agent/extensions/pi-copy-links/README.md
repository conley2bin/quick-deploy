...
- 按下按钮或 Ctrl 按下链接时目标会反色高亮，松开即恢复；拖走会立即取消高亮且不触发动作。复制成功后按钮短暂闪烁确认。
...
- 扩展只在首次渲染某个 Markdown 实例时登记一次弱引用（用于按下/闪烁时精确失效缓存）。V8 对单个 Set 有 2^24 上限，逐次渲染登记会在长时间流式输出后把 Pi 打崩，因此登记次数必须与存活实例数同阶。若登记因进程级内存耗尽失败，扩展会降级为不装饰的普通 Markdown，并在后续事件循环里最多重试 8 次；仍失败则通知并停用（重启 Pi 恢复），不会把异常抛回渲染器。
