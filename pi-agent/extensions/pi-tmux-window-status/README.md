...
- Both selected and unselected windows breathe the same gray palette `#bcbcbc → #f5f5f5 → #bcbcbc → #808080 → #bcbcbc`. Selection is no longer a blue background block; instead the selected tab draws a pair of fixed deep-blue `#0077aa` half-block side rails, `▐` on the left and `▌` on the right, both solid halves facing inward in a powerline-cradle style — a visual channel orthogonal to the background, so a selected window with a latched error shows the red block plus the blue rails instead of losing the selection cue. The rails never breathe and never change color.
