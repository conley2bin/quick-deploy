...
- Both selected and unselected windows breathe the same gray palette `#bcbcbc → #f5f5f5 → #bcbcbc → #808080 → #bcbcbc`. Selection is no longer a blue background block; instead the selected tab draws a bold deep-blue `#0077aa` box frame with all four corners: the top status line carries `┏━` content `━┓` and the bottom line `┗━…━┛`. A single status line has no room for a bottom bar, so the selected tab spans two status lines while unselected tabs keep one; the bottom bar maps the top content through an `s/` substitution that turns every non-space character into `━`, keeping both lines flush for any window name length without padding or truncation. The frame never breathes and never changes color, and a selected window with a latched error shows the red block (both lines) plus the blue frame instead of losing the selection cue.
