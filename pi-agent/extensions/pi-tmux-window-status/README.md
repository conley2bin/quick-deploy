...
- Both selected and unselected windows breathe the same gray palette `#bcbcbc → #f5f5f5 → #bcbcbc → #808080 → #bcbcbc`. Selection is no longer a blue background block; instead the selected tab draws a full-block `#0077aa` bar at each end (`█`, with both fg and bg blue so even font-rendered gaps never show the block color) — a visual channel orthogonal to the background, so a selected window with a latched error shows the red block plus solid blue ends instead of losing the selection cue. The bars never breathe and never change color.
