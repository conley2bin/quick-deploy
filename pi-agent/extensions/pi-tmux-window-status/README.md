...
- Both selected and unselected windows breathe the same gray palette `#bcbcbc → #f5f5f5 → #bcbcbc → #808080 → #bcbcbc`. Selection is no longer a blue background block; instead the selected tab draws a bold deep-blue `#0077aa` box frame (`┏━` left corner/bar and `━┓` right bar/corner) around the block, a visual channel orthogonal to the background, so a selected window with a latched error shows the red block plus the blue frame instead of losing the selection cue. The frame never breathes and never changes color. A true bottom bar is physically impossible in the one-line tmux status bar; `┏━…━┓` is the closest complete-box approximation a single line can draw.
