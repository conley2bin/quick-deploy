...
await runner.emit({type:'session_compact_failed',reason:'manual',errorMessage:'fixture failure',aborted:false,willRetry:false,fromExtension:false});await settle();report.compactionFailed=snapshot();
await runner.emit({type:'session_compact_failed',reason:'manual',aborted:true,willRetry:false,fromExtension:false});await settle();report.compactionCancelled=snapshot();
...
console.log('COMPACTION_JSON '+JSON.stringify(report));
