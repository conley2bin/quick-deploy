...
import { completeUploadTransaction, DEFAULT_MAX_TRANSACTION_BYTES, type TransportScheduler, type TransportSink } from "../src/transport.ts";
...
test("preview derivation downsizes without enlargement and bounds PNG plus complete wire bytes", async () => {
  const width = 1_024;
  const height = 768;
  ...
  const source = await sharp(raw, { raw: { width, height, channels: 3 } }).png({ compressionLevel: 0 }).toBuffer();
  const before = Buffer.from(source);
  const loaded = await loadImage(`data:image/png;base64,${source.toString("base64")}`, "/fixture");

  assert.deepEqual(source, before, "preview work never edits the source buffer");
  assert.deepEqual([loaded.width, loaded.height], [width, height]);
  assert.ok(loaded.previewWidth <= MAX_PREVIEW_WIDTH_PX);
  assert.ok(loaded.previewHeight <= MAX_PREVIEW_HEIGHT_PX);
  assert.ok(loaded.previewWidth * loaded.previewHeight <= MAX_PREVIEW_PIXELS);
  assert.ok(loaded.previewWidth < width && loaded.previewHeight < height);
  assert.ok(loaded.png.length <= MAX_PREVIEW_PNG_BYTES);
  assert.equal(
    Buffer.byteLength(completeUploadTransaction(Buffer.alloc(MAX_PREVIEW_PNG_BYTES), 0xffffffff, true)),
    878_123,
    "640 KiB PNG cap fixes the maximum normal tmux upload burst",
  );

  const { sink, terminal } = runtime({ minIntervalMs: 0 });
  await terminal.prepare("large", loaded);
  assert.ok(Buffer.byteLength(sink.writes[0]!.value) <= DEFAULT_MAX_TRANSACTION_BYTES);
...
test("worst catalog deduplicates widths to 80 entries and remains within its metadata budget", async () => {
  ...
  const catalog = sink.writes[1]!.value;
  ...
  assert.equal(Buffer.byteLength(catalog), 4_432);
  assert.equal(MAX_PLACEMENT_CATALOG_BYTES, 4_480);
  assert.ok(Buffer.byteLength(catalog) <= MAX_PLACEMENT_CATALOG_BYTES);
  await terminal.clear(true);
});
...
test("cell metric changes fail explicitly instead of selecting a missing placement catalog entry", async () => {
  ...
  const { sink, terminal } = runtime({ minIntervalMs: 0, cell });
...
test("old-plugin graphics can occur only between complete inline upload writes", async () => {
  ...
  assert.ok(firstApcs.length > 2 && firstApcs.at(-1)!.get("m") === "0");
  ...
  assert.equal(graphicsCommands(sink.writes[secondUploadIndex]!.value).at(-1)!.get("m"), "0");
...
test("reset rejects late loads and cancels backpressured placements before ordered deletion", async () => {
  ...
  assert.equal(blocked.terminal.pendingJobs(), 1, "placement catalog waits behind accepted-false upload");
  ...
  assert.equal(blocked.sink.writes.some(({ value }) => graphicsCommands(value).some((command) => command.get("a") === "p")), false);
  ...
  assert.equal(blocked.sink.writes.some(({ value }) => graphicsCommands(value).some((command) => command.get("a") === "p")), false, "cancelled catalog never writes later");
