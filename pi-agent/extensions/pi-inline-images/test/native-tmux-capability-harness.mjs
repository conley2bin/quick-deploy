...
const core=process.env.PI_HOST_ROOT;if(!core)throw new Error('PI_HOST_ROOT is required');
...
const formats=process.argv[3]?[process.argv[3]]:['jpeg','webp','png'];
const history=[];
...
for(const row of mode.chatContainer.children)if(row instanceof ToolExecutionComponent)row.setShowImages(false);fakeTui.invalidate();paint();await settle();report.externalOff=snapshot();
for(const row of mode.chatContainer.children)if(row instanceof ToolExecutionComponent)row.setShowImages(true);fakeTui.invalidate();paint();await settle();report.externalOn=snapshot();
...
report.rapid={detectedTmux,history,final:snapshot(),frames};
await runner.emit({type:'session_shutdown'});await settle();
console.log('TMUX_JSON '+JSON.stringify(report));
